...
const transporter = nodemailer.createTransport({
  ...
  port: emailPort,
  secure: emailPort === 465, // use true for SSL (465), false for STARTTLS (587)
  auth: {
...
export async function sendEmail(to: string, subject: string, html: string, text?: string): Promise<void> {
  ...
  let cleanFrom = rawFrom.replace(/^"+|"+$/g, '').trim();

  // Guard against misconfigured EMAIL_FROM (e.g. only a name without email brackets)
  if (cleanFrom && !cleanFrom.includes('<') && !cleanFrom.includes('@')) {
    cleanFrom = `"${cleanFrom}" <${defaultFromEmail}>`;
  }
