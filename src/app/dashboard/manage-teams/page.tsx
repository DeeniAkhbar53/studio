...
export default function ManageTeamsPage() {
  ...
  const handleMoveMember = async (memberId: string, newTeam: string | null) => {
    ...
    try {
        const payload = { team: (newTeam === "unassigned" || !newTeam) ? "" : newTeam };
        await updateUser(memberId, member.mohallahId, payload);
        
        // Optimistically update UI
        setUsers(prevUsers => prevUsers.map(u => u.id === memberId ? { ...u, team: (newTeam === "unassigned" || !newTeam) ? undefined : newTeam } : u));
